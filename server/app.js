...
import { productionDashboard, listProductionOrders, createProductionOrder, updateProductionOrder, productionHealth } from './production.js';
...
export async function handle(req, res) {
  ...
  const url = route(req);
  try {
    if (req.method === 'POST' && url.pathname === '/api/billing/webhook') return json(res, 200, await handleStripeWebhook(await rawBody(req), req.headers['stripe-signature']), cors);
    ...
    if (req.method === 'GET' && (url.pathname === '/health' || url.pathname === '/api/health')) return json(res, 200, { ok: true, ...readiness(), production: await productionHealth() }, cors);
    if (req.method === 'GET' && url.pathname === '/api/ready') { const status = readiness(); const ready = status.supabase && status.adminAuth; return json(res, ready ? 200 : 503, { ready, ...status, production: await productionHealth() }, cors); }
    ...
    if (url.pathname === '/api/production/dashboard' && req.method === 'GET') return json(res, 200, await productionDashboard(url.searchParams.get('organizationId')), cors);
    if (url.pathname === '/api/production/orders' && req.method === 'GET') return json(res, 200, await listProductionOrders(url.searchParams.get('organizationId'), url.searchParams.get('status')), cors);
    if (url.pathname === '/api/production/orders' && req.method === 'POST') { if (!auth(req)) return json(res, 401, { error: 'admin_auth_required' }, cors); return json(res, 201, await createProductionOrder(await body(req)), cors); }
    if (url.pathname.startsWith('/api/production/orders/') && req.method === 'PATCH') { if (!auth(req)) return json(res, 401, { error: 'admin_auth_required' }, cors); return json(res, 200, await updateProductionOrder(url.pathname.split('/').pop(), await body(req)), cors); }
    if (req.method === 'POST' && url.pathname === '/api/billing/checkout') { const data = await body(req); return json(res, 201, await createCheckout({ planKey: data.planKey, companyName: data.companyName, email: data.email }), cors); }
