...
function allowed(req) {
  const now = Date.now(), key = req.socket?.remoteAddress || req.headers['x-forwarded-for'] || 'unknown';
...
function originHeaders(req) {
  ...
  if (!configured || !requestOrigin || requestOrigin === configured) return configured ? { 'access-control-allow-origin': configured, 'vary': 'Origin' } : {};
  return {};
}
function readiness() {
  return {
    service: 'marmopro-nucleo',
    supabase: Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY),
    ai: Boolean(process.env.AI_API_URL && process.env.AI_API_KEY && process.env.AI_MODEL),
    ...
    adminAuth: Boolean(process.env.ADMIN_TOKEN),
...
export async function handle(req, res) {
  ...
  if (req.method === 'OPTIONS') {
    return json(res, 204, null, { ...cors, 'access-control-allow-methods': 'GET,POST,OPTIONS', 'access-control-allow-headers': 'Content-Type, Authorization', 'access-control-max-age': '600' });
  }
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/') {
      ...
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'x-content-type-options': 'nosniff', 'x-frame-options': 'DENY', 'referrer-policy': 'no-referrer', ...cors });
      return res.end(html);
    }
    if (req.method === 'GET' && (url.pathname === '/health' || url.pathname === '/api/health')) return json(res, 200, { ok: true, ...readiness() }, cors);
    if (req.method === 'GET' && url.pathname === '/api/ready') {
      const status = readiness();
      const ready = status.supabase && status.adminAuth;
      return json(res, ready ? 200 : 503, { ready, ...status }, cors);
    }
    if (req.method === 'GET' && url.pathname === '/api/dashboard') return json(res, 200, { leads: store.leads.length, conversations: store.conversations.length, messages: store.messages.length, campaigns: store.campaigns.length, escalations: store.conversations.filter(c => c.escalated).length }, cors);
    ...
    if (req.method === 'POST' && url.pathname === '/api/billing/checkout') {
      ...
      return json(res, 201, result, cors);
    }

    if (req.method === 'POST' && url.pathname === '/api/leads') {
      const data = await body(req); if (!data.name && !data.phone && !data.email) return json(res, 400, { error: 'lead_identifier_required' }, cors);
      ...
      store.leads.push(lead); await saveStore(store); return json(res, 201, lead, cors);
    }

    if (req.method === 'POST' && url.pathname === '/api/chat') {
      const data = await body(req); if (!data.message || typeof data.message !== 'string') return json(res, 400, { error: 'message_required' }, cors);
      ...
      await saveStore(store); return json(res, 200, { conversationId, ...answer }, cors);
    }

    if (req.method === 'POST' && url.pathname === '/api/campaigns') {
      if (!auth(req)) return json(res, 401, { error: 'admin_auth_required' }, cors);
      const campaign = createCampaign(await body(req));
      store.campaigns.push(campaign); await saveStore(store); return json(res, 201, campaign, cors);
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/campaigns/metrics') {
      ...
      if (!campaign) return json(res, 404, { error: 'campaign_not_found' }, cors);
      updateCampaignMetrics(campaign, data); await saveStore(store); return json(res, 200, campaign, cors);
    }

    if (req.method === 'POST' && url.pathname === '/api/marketing/brief') {
      if (!auth(req)) return json(res, 401, { error: 'admin_auth_required' }, cors);
      return json(res, 200, generateMarketingBrief(await body(req)), cors);
    }

    if (req.method === 'POST' && url.pathname === '/api/messages/send') {
      if (!auth(req)) return json(res, 401, { error: 'admin_auth_required' }, cors);
      const data = await body(req); if (!data.channel || !data.recipient || !data.text) return json(res, 400, { error: 'channel_recipient_text_required' }, cors);
